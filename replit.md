...
- **Admin Panel:** SmartHR-style control panel with unified list→detail patterns across all 15 screens. Shared component library in `client/src/components/admin/` (AdminPageHeader, AdminStatCards, AdminFilterBar, AdminStatusBadge, AdminActionBar, AdminDetailLayout, AdminEmptyState). Every list page follows: Header + Stat Cards + Filter Tabs + Search + DataTable with row selection, bulk actions, and pagination. Every detail page uses AdminDetailLayout with avatar, badges, action dropdown, tabs, and prev/next navigation. Sidebar navigation with group labels (Overview, Sales & Clients, Content Library, Team).
