...
- **Admin Panel:** Redesigned to match HR-Harmony-Hub reference. Uses Plus Jakarta Sans font (--font-heading), blue primary (HSL 223 83% 53%), collapsible sidebar with SidebarProvider pattern, topbar with search/notifications/user menu. Shared components in `client/src/components/admin-shared/`: PageShell, PageHeader, HeroBanner, StatCard/StatGrid, DataTable (search/sort/filter/pagination/row actions), StatusBadge, EmptyState, FormDialog. All admin pages rebuilt using DataTable pattern. Routes for all admin sections registered in App.tsx (categories, suppliers, leads, plans, competitor-stores, shopify-stores).
