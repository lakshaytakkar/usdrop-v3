...
- **Admin Panel:** Streamlined control panel with 4 sections: Dashboard | Clients | Courses | Team. Top horizontal navigation bar (`AdminTopNavigation`). Shared component library in `client/src/components/admin/` (AdminPageHeader, AdminStatCards, AdminFilterBar, AdminStatusBadge, AdminActionBar, AdminDetailLayout, AdminEmptyState). Clients list page has Create/Edit User modal, user detail page has Edit User dialog, Reset Password action, and per-module access control (Sessions tab). Course builder is a 2-panel layout (left: module/lesson tree, right: content editor with YouTube URL embed or direct video upload to Supabase Storage `course-videos` bucket). Courses list is a clean table with create/publish/delete actions.
...
- **Admin Panel Rebuild (SmartHR-style)**: Complete rebuild of all 15 admin screens into a unified control panel. Shared admin component library (`client/src/components/admin/`) with 7 reusable components: `AdminPageHeader` (breadcrumbs, title, actions), `AdminStatCards` (metric grid), `AdminFilterBar` (tabs + search), `AdminStatusBadge` (colored status badges), `AdminActionBar` (bulk actions toolbar), `AdminDetailLayout` (detail page shell with tabs + nav), `AdminEmptyState` (empty table CTA). All list pages rebuilt: Dashboard, Clients, Leads, Plans, Products, Courses, Categories, Competitors, Suppliers, Shopify Stores, Team. All detail pages rebuilt: Client Detail (with Journey/Sessions/Activity/Notes tabs), Lead Detail, Plan Detail (with Permission Matrix/Subscribers tabs), Product Detail (with inline editing), Category Detail, Competitor Detail, Team Detail (with Permission Matrix). New backend endpoints: `GET/PATCH /api/admin/external-users/:id/sessions` (module access overrides), `GET /api/admin/external-users/:id/journey`, `GET /api/admin/external-users/:id/activity`, `POST/GET /api/admin/external-users/:id/notes`. New Supabase tables: `user_admin_notes`, `user_module_overrides`.
- **Admin Top Navigation**: Replaced sidebar-based admin layout with a horizontal top navigation bar. `AdminTopNavigation` component (`client/src/components/layout/admin-top-navigation.tsx`) with nav groups (Overview | Sales & Clients | Content Library | Team) separated by subtle dividers. `AdminLayout` now uses `AdminTopNavigation` + full-width content area instead of `SidebarProvider + AppSidebar`. Admin nav items removed from `app-sidebar.tsx`. Mobile responsive with hamburger slide-out menu. User avatar dropdown with role badge.
- **404 Smart Redirect**: NotFound page auto-redirects based on user role after 5s countdown. Admin users → `/admin`, regular users → `/framework`, guests → `/login`. Guards loading state to avoid premature redirects. Includes manual "Go to" button.
- **Admin Login Auto-Redirect**: `getUserRedirectPath` returns `/admin` for internal users. Login fallback defaults to `/framework` (avoids loop).
- **Admin Panel Streamlining**: Reduced admin nav from 15+ screens to 4 focused sections: Dashboard, Clients, Courses, Team. Removed Plans, Products, Categories, Competitors, Suppliers, Shopify Stores, Leads from navigation and routes. Course builder rebuilt as a 2-panel layout with module/lesson tree (left) and content editor (right). Video lessons support YouTube URL embed or direct file upload to Supabase Storage `course-videos` bucket with XHR progress bar. Courses list simplified to clean table with create/publish/delete. User detail page enhanced with Edit User dialog (name, email, phone, plan, status) and Reset Password action with password strength indicator and generate/copy features. Sessions/Notes endpoints handle missing Supabase tables gracefully. Files: `admin-top-navigation.tsx`, `App.tsx`, `courses/page.tsx`, `courses/[courseId]/builder/page.tsx`, `external-users/[id]/page.tsx`, `server/routes/admin.ts`.