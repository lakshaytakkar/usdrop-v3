...
export async function GET(request: NextRequest) {
  try {
    ...
    const products = data.map((row: any) => ({
      ...
      additional_images: typeof row.additional_images === 'string' ? JSON.parse(row.additional_images) : (row.additional_images || []),
      ...
      metadata: row.meta_id ? {
        ...
        revenue_trend: typeof row.meta_revenue_trend === 'string' ? JSON.parse(row.meta_revenue_trend) : (row.meta_revenue_trend || []),
        found_date: row.meta_found_date,
        detailed_analysis: typeof row.meta_detailed_analysis === 'string' ? JSON.parse(row.meta_detailed_analysis) : row.meta_detailed_analysis,
        filters: typeof row.meta_filters === 'string' ? JSON.parse(row.meta_filters) : (row.meta_filters || []),
