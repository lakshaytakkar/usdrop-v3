...
export async function POST(request: Request) {
  try {
    ...
    if (!ecommerce_experience) {
      return NextResponse.json({ error: 'Ecommerce experience is required' }, { status: 400 })
    }

    if (!preferred_niche) {
      return NextResponse.json({ error: 'Preferred niche is required' }, { status: 400 })
    }
    ...
    if (!validExperiences.includes(ecommerce_experience)) {
      return NextResponse.json({ error: 'Invalid ecommerce experience value' }, { status: 400 })
    }
    ...
    if (!validNiches.includes(preferred_niche)) {
      return NextResponse.json({ error: 'Invalid preferred niche value' }, { status: 400 })
    }
