...
function mapRowToProduct(row: any) {
  return {
    ...
    additional_images: (() => { try { const v = typeof row.additional_images === 'string' ? JSON.parse(row.additional_images) : row.additional_images; return Array.isArray(v) ? v : []; } catch { return []; } })(),
    specifications: (() => { try { return typeof row.specifications === 'string' ? JSON.parse(row.specifications) : (row.specifications || null); } catch { return null; } })(),
    rating: row.rating ? parseFloat(row.rating) : null,
    reviews_count: row.reviews_count || 0,
    trend_data: (() => { try { const v = typeof row.trend_data === 'string' ? JSON.parse(row.trend_data) : row.trend_data; return Array.isArray(v) ? v : []; } catch { return []; } })(),
