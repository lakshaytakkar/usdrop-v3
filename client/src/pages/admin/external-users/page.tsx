...
import { Plus, Trash2, Check, Lock, Search, X, Mail, MessageCircle, Coins, Copy, Eye, EyeOff, RefreshCw, ChevronLeft, ChevronRight, Download, Edit, MoreVertical, UserPlus, Upload, AlertCircle, Filter, Users, CreditCard, UserCog } from "lucide-react"
...
export default function ExternalUsersPage() {
  ...
  const confirmDelete = async () => {
    ...
    try {
      const response = await apiFetch(`/api/admin/external-users/${userToDelete.id}`, {
        method: "DELETE",
      })
  ...
  return (
    <div className="flex flex-1 flex-col min-w-0 h-full overflow-hidden">
      <div className="flex items-center justify-between mb-1">
        <div>
          <h1 className="text-xl font-semibold leading-[1.35] tracking-tight text-foreground">Clients</h1>
          <p className="text-sm text-muted-foreground mt-1">Manage platform clients and their progress</p>
        </div>
      </div>
      ...
      {!initialLoading && users.length > 0 && (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
          <div className="bg-card border rounded-lg p-4 shadow-[0px_1px_2px_0px_rgba(13,13,18,0.06)] dark:shadow-none">
            ...
            <div className="mt-2">
              <span className="text-xs text-muted-foreground">All platform clients</span>
            </div>
          </div>
          <div className="bg-card border rounded-lg p-4 shadow-[0px_1px_2px_0px_rgba(13,13,18,0.06)] dark:shadow-none">
            <div className="flex items-center justify-between">
              <span className="text-sm font-medium text-muted-foreground">Pro Users</span>
              <div className="w-9 h-9 rounded-lg border flex items-center justify-center">
                <CreditCard className="h-[18px] w-[18px] text-primary" />
              </div>
            </div>
            <div className="mt-1">
              <span className="text-2xl font-semibold">{users.filter(u => u.plan === 'pro').length}</span>
            </div>
            <div className="mt-2">
              <span className="text-xs text-muted-foreground">Users on pro plan</span>
            </div>
          </div>
          <div className="bg-card border rounded-lg p-4 shadow-[0px_1px_2px_0px_rgba(13,13,18,0.06)] dark:shadow-none">
            <div className="flex items-center justify-between">
              <span className="text-sm font-medium text-muted-foreground">Free Users</span>
              <div className="w-9 h-9 rounded-lg border flex items-center justify-center">
                <UserCog className="h-[18px] w-[18px] text-primary" />
              </div>
            </div>
            <div className="mt-1">
              <span className="text-2xl font-semibold">{users.filter(u => u.plan === 'free').length}</span>
            </div>
            <div className="mt-2">
              <span className="text-xs text-muted-foreground">Users on free plan</span>
            </div>
          </div>
        </div>
      )}
