...
- **Feb 2026**: Separated "Tools" into two top-level nav groups: **AI Studio** (5 creative AI tools: Model Studio, Whitelabelling, Description Generator, Email Templates, Policy Generator at `/ai-studio/*`) and **Tools** (3 utility tools: Invoice Generator, Profit Calculator, Shipping Calculator at `/tools/*`). Removed header "More" dropdown. Resources page converted from card grid to list/table layout. Header nav font reduced to 14px to fit all items.
