...
if (!GEMINI_API_KEY) {
  ...
  console.log("Gemini API key found, using Gemini for product research")
}
...
const mockResearchProduct = async (input: ResearchInput): Promise<Partial<ProductResearch>> => {
...
export const productResearchService = {
  /**
   * Research a product using Gemini API to extract structured insights
   */
  researchProduct: async (input: ResearchInput): Promise<Partial<ProductResearch>> => {
    if (!genAI) {
