...
## Recent Changes
- **Add Product Flow (My Products)**: Full CRUD flow for creating products. Choice modal with two options: "Import from URL" (scrapes OG tags to auto-fill title/image/description) and "Enter Manually" (form with name, category dropdown, description, buy/sell prices, image URL). Backend endpoints: `POST /api/products/create-manual` and `POST /api/products/import-url` in `server/routes/public.ts`. Both create a product in `products` table and link to `user_picklist` (with new `source` column). Search bar standardized to white bg with "Add Product" button.
