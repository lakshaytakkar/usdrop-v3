...
function processRow(row: CSVRow): ProcessedProduct | null {
...
function removeDuplicates(products: ProcessedProduct[]): ProcessedProduct[] {
  const seen = new Set<string>();
  const unique: ProcessedProduct[] = [];
...
function processCSVToProducts() {
  ...
  // CSV file moved to archive-unused-files/csv-data/ - update path if needed
  const csvPath = path.join(__dirname, '../archive-unused-files/csv-data/snap_miin3sjp1o36majndf.csv');
