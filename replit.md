...
- **Trending Products**: Uses the full scraped product catalog (same as Product Hunt) with a different daily shuffle seed (`shuffle=daily-trending`). Generates deterministic trend chart data per product per day. Infinite scroll (6 items per load, IntersectionObserver).
- **Starred Products**: Admin can star/shortlist products in a user's picklist via `PATCH /api/admin/users/:userId/picklist/:picklistId/star`. Starred status stored as JSON in the `notes` column of `user_picklist` (e.g., `{"starred":true}`). Starred products appear first in both user's My Products page and admin user detail panel. My Products has a "Starred" tab to filter only starred items. Admin sees clickable star toggles in the products table.
