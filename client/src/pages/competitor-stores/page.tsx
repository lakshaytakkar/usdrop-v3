...
export default function CompetitorStoresPage() {
  ...
  const fetchStores = useCallback(async () => {
    try {
      ...
      const response = await apiFetch(`/api/competitor-stores?${params.toString()}`, {
        credentials: 'include'
      })
      ...
      <>
          <div className="flex flex-1 flex-col gap-2 p-4 md:p-6 bg-gray-50/50 min-h-0 relative">
            <div className="rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 px-6 py-5 text-white">
              <h1 className="text-2xl font-bold">Competitor Stores</h1>
              <p className="text-sm text-blue-100 mt-1">Analyze top-performing Shopify stores and market trends</p>
            </div>
