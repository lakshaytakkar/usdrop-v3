...
export const GeneratedImage: React.FC<GeneratedImageProps> = ({ prompt, className = "", alt = "Generated Content", overlay = false }) => {
  ...
  useEffect(() => {
    ...
    if (!apiKey) {
      console.warn("No Gemini API key found. Image generation disabled. Set NEXT_PUBLIC_GEMINI_API_KEY in your .env file.");
      return;
    }
    ...
    const timeoutId = setTimeout(() => {
      if (observerTarget.current) {
        ...
        if (isVisible && !hasLoaded.current) {
          // Element is already visible, generate immediately
          generateImage();
        } else {
          // Element not visible yet, observe for when it becomes visible
