...
- **PWA Support**: App is a Progressive Web App. `client/public/manifest.json` for installability, `client/public/sw.js` service worker for caching (production only). `MobileDesktopBanner` component shows "open on PC" notice on mobile (< 768px, session-dismissible). `PwaInstallPrompt` component shows native install prompt on supported browsers. Both in `client/src/components/layout/`. Added to `AppLayout.tsx`. Apple glass-effect icon at `client/public/pwa-icon-512.png`.