...
import { Play, ChevronDown, ChevronRight, Clock, Lock, Video, BookOpen, AlertCircle } from "lucide-react"
...
function ModuleLessonCard({
  ...
  const thumbnail = module.thumbnail || getYouTubeThumbnail(module.content?.video_url || module.video_url)
  ...
  const inner = (
    <div
      data-testid={`card-lesson-${module.id}`}
      className={cn(
    ...
    >
      <div className="w-[200px] min-w-[200px] h-[120px] relative bg-gray-100 overflow-hidden shrink-0">
        {thumbnail && !imgError ? (
          <img
            ...
            data-testid={`img-lesson-${module.id}`}
          />
        ) : (
      ...
      <div className="flex-1 min-w-0 px-4 py-3 flex flex-col justify-between">
        <div>
          <h4 className="text-sm font-semibold text-gray-900 group-hover:text-blue-600 transition-colors line-clamp-1" data-testid={`text-lesson-title-${module.id}`}>
  ...
  if (locked) {
    return (
      <div className="cursor-pointer" onClick={onLockedClick} data-testid={`link-lesson-${module.id}`}>
        {inner}
      </div>
    )
  }
  ...
  return (
    <Link href={`/framework/my-learning/${courseId}?module=${module.id}`} data-testid={`link-lesson-${module.id}`}>
      {inner}
    </Link>
  )
}
    ...
    <div className="rounded-xl overflow-hidden" data-testid={`course-${course.id}`}>
      ...
      >
        <div className="flex items-center gap-3 min-w-0">
          ...
          <span className="text-[15px] font-semibold text-left truncate">{course.title}</span>
          {locked && (
        ...
        <div className="flex items-center gap-3 shrink-0 ml-3">
          <span className="text-xs text-gray-400 font-medium">{modules.length} Lesson{modules.length !== 1 ? "s" : ""}</span>
          {totalChapters > 0 && (
            <>
              <span className="text-gray-200">·</span>
              <span className="text-xs text-gray-400 font-medium">{totalChapters} Chapter{totalChapters !== 1 ? "s" : ""}</span>
            </>
          )}
          {duration && (
            <>
      ...
      <div className="flex flex-1 flex-col gap-6 px-12 md:px-20 lg:px-32 py-6 md:py-8">
        ...
        <div className="flex items-center justify-between">
          ...
          <span className="text-sm text-gray-400 font-medium">
            {courses.length} course{courses.length !== 1 ? "s" : ""} · {totalModules} lessons
          </span>
        </div>
