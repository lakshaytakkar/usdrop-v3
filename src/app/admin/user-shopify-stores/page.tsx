...
import { AdminShopifyStoreCard } from "@/app/admin/shopify-stores/components/admin-shopify-store-card"
...
export default function AdminCompetitorStoresPage() {
  ...
  const canVerify = false // ShopifyStore doesn't have a verified property
  ...
  const [statusTab, setStatusTab] = useState<"all" | "connected" | "disconnected" | "syncing" | "error" | "high_traffic" | "high_revenue" | "high_growth">("all")
  ...
  const categories = useMemo(() => {
    const categorySet = new Set(stores.map((s) => s.niche).filter(Boolean))
  ...
  const getStatusCount = useCallback((tab: typeof statusTab) => {
    switch (tab) {
      ...
      case "disconnected":
        return stores.filter(s => s.status === "disconnected").length
      case "syncing":
        return stores.filter(s => s.status === "syncing").length
      case "error":
        return stores.filter(s => s.status === "error").length
      default:
        return 0
    }
  }, [stores])
  ...
  const filteredStores = useMemo(() => {
    ...
    if (statusTab !== "all") {
      switch (statusTab) {
        case "high_traffic":
          result = result.filter(s => s.monthly_traffic !== null && s.monthly_traffic >= 100000)
          break
        case "high_revenue":
          result = result.filter(s => s.monthly_revenue !== null && s.monthly_revenue >= 400000)
          break
        case "high_growth":
          // ShopifyStore doesn't have a growth property, skip this filter
          break
      }
    }
    ...
    if (quickFilter) {
      switch (quickFilter) {
        ...
        case "high_growth":
          ...
          break
        case "verified":
          // ShopifyStore doesn't have a verified property, skip this filter
          break
      }
    }
    ...
    if (searchQuery) {
      ...
      result = result.filter((store) =>
        ...
        (store.niche && store.niche.toLowerCase().includes(searchLower)) ||
    ...
    columnFilters.forEach((filter) => {
      ...
      if (filter.id === "category") {
        result = result.filter((store) => store.niche && filterValues.includes(store.niche))
      }
      ...
      sorted.sort((a, b) => {
        ...
        switch (id) {
          ...
          case "monthly_traffic":
            aValue = a.monthly_traffic || 0
            bValue = b.monthly_traffic || 0
            break
          case "monthly_revenue":
            ...
            break
          case "growth":
            // ShopifyStore doesn't have a growth property, use 0 for neutral sort
            aValue = 0
            bValue = 0
            break
          case "country":
            ...
            break
          case "rating":
            // ShopifyStore doesn't have a rating property, use 0 for neutral sort
            aValue = 0
            bValue = 0
            break
          default:
  ...
  const handleViewDetails = useCallback((store: ShopifyStore) => {
    router.push(`/admin/user-shopify-stores/${store.id}`)
  }, [router])

  const handleQuickView = useCallback((store: ShopifyStore) => {
    setSelectedStore(store)
    setQuickViewOpen(true)
  }, [])

  const handleDelete = useCallback((store: ShopifyStore) => {
    if (!canDelete) {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : "Failed to delete store. Please try again."
      showError(errorMessage)
    } finally {
  ...
  const handleCopyStoreId = useCallback(async (store: ShopifyStore) => {
    try {
  ...
  const handleCopyUrl = useCallback(async (store: ShopifyStore) => {
    try {
  ...
  const handleSync = useCallback(async (store: ShopifyStore) => {
    try {
      ...
      showSuccess(`${store.name} synced successfully`)
      await fetchStores()
    } catch (err) {
  ...
  }, [showSuccess, showError, fetchStores])

  const handleVisitStore = useCallback((store: ShopifyStore) => {
    const url = store.url.startsWith("http") ? store.url : `https://${store.url}`
    window.open(url, "_blank", "noopener,noreferrer")
  }, [])

  const handleViewProducts = useCallback((store: ShopifyStore) => {
    router.push(`/admin/products?store=${store.id}`)
  }, [router])

  const handleEdit = useCallback((store: ShopifyStore) => {
    if (!canEdit) {
    ...
    setFormData({
      name: store.name,
      url: store.url,
      category: store.niche || "",
      country: store.country || "",
      monthly_traffic: store.monthly_traffic || 0,
      monthly_revenue: store.monthly_revenue,
      growth: 0, // ShopifyStore doesn't have a growth property
      products_count: store.products_count,
      rating: undefined, // ShopifyStore doesn't have a rating property
      verified: false, // ShopifyStore doesn't have a verified property
      logo: store.logo || "",
    })
    setFormErrors({})
    setFormOpen(true)
  ...
  const handleDuplicate = useCallback(async (store: ShopifyStore) => {
    if (!canCreate) {
    ...
    try {
      ...
      const duplicatedStore: ShopifyStore = {
        ...store,
        id: `store_${Date.now()}`,
          ...
          prev.map((s) =>
            s.id === editingStore.id
              ? {
                  ...
                  name: formData.name,
                  url: formData.url,
                  niche: formData.category || null,
                  country: formData.country || null,
                  monthly_traffic: formData.monthly_traffic || null,
                  monthly_revenue: formData.monthly_revenue,
                  products_count: formData.products_count || 0,
  ...
  const handleBulkExport = useCallback(() => {
    ...
    try {
      const csv = [
        ["Store ID", "Name", "URL", "Niche", "Country", "Monthly Traffic", "Monthly Revenue", "Products", "Status"],
  ...
  const handleRowClick = useCallback((store: ShopifyStore) => {
    setSelectedStore(store)
    setQuickViewOpen(true)
  }, [])
    ...
    <div className="flex flex-1 flex-col min-w-0 h-full overflow-hidden">
      <div className="bg-primary/85 text-primary-foreground rounded-md px-4 py-3 mb-3 flex-shrink-0 w-full">
        <div className="flex items-center justify-between flex-wrap gap-2">
          <div>
            <h1 className="text-lg font-semibold tracking-tight text-white">User Shopify Stores</h1>
            <p className="text-xs text-white/90 mt-0.5">
              Manage user-connected Shopify stores
            </p>
          </div>
                ...
                {paginatedStores.map((store) => (
                  <AdminShopifyStoreCard
                    ...
                    onSync={async (s) => {
                      try {
                        setLoading(true)
                        await fetch(`/api/shopify-stores/${s.id}/sync`, { method: 'POST' })
                        showSuccess(`${s.name} synced successfully`)
                        await fetchStores()
                      } catch (err) {
                        showError('Failed to sync store')
                      } finally {
                        setLoading(false)
                      }
                    }}
        ...
        >
          <DialogContent className="max-w-xs p-4">
            ...
            <div className="space-y-2 py-2">
              ...
              {selectedStore.monthly_traffic !== null && (
                <div className="flex items-center justify-between gap-2">
              ...
              {selectedStore.monthly_revenue !== null && (
                <div className="flex items-center justify-between gap-2">
              ...
              <div className="flex items-center justify-between gap-2">
                ...
                <Badge variant={selectedStore.status === "connected" ? "default" : "outline"} className="text-xs px-2 py-0.5">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    {selectedStore.niche && (
                      <div>
                        ...
                        <Badge variant="outline">{selectedStore.niche}</Badge>
                      </div>
                    )}
                    {selectedStore.country && (
                      <div>
                    ...
                    <div>
                      ...
                      <Badge variant={selectedStore.status === "connected" ? "default" : "outline"}>
                        {selectedStore.status.charAt(0).toUpperCase() + selectedStore.status.slice(1)}
                      </Badge>
                    </div>
                    <div>
                      ...
                      <p className="text-lg font-semibold">
                        {selectedStore.monthly_traffic !== null ? numberFormatter.format(selectedStore.monthly_traffic) : "—"}
                      </p>
                    </div>
                    <div>
                      <p className="text-sm text-muted-foreground mb-1">Monthly Revenue</p>
                      <p className="text-lg font-semibold">
                        {selectedStore.monthly_revenue !== null ? currencyFormatter.format(selectedStore.monthly_revenue) : "—"}
                      </p>
                    </div>
                    {(selectedStore as any).growth !== undefined && (selectedStore as any).growth !== null && (
                      <div>
                        <p className="text-sm text-muted-foreground mb-1">Growth</p>
                        <p className={`text-lg font-semibold ${(selectedStore as any).growth > 0 ? "text-emerald-600" : "text-destructive"}`}>
                          {(selectedStore as any).growth > 0 ? "+" : ""}{(selectedStore as any).growth.toFixed(1)}%
                        </p>
                      </div>
                    )}
                    ...
                    {(selectedStore as any).rating !== undefined && (selectedStore as any).rating !== null && (
                      <div>
                        <p className="text-sm text-muted-foreground mb-1">Rating</p>
                        <div className="flex items-center gap-1">
                          <Star className="h-4 w-4 fill-yellow-400 text-yellow-400" />
                          <span className="text-lg font-semibold">{(selectedStore as any).rating.toFixed(1)}</span>
                        </div>
                      </div>
                    )}
                  </div>
                ...
                <div className="space-y-4">
                  ...
                  {(selectedStore as any).growth !== undefined && (selectedStore as any).growth !== null && (
                    <div>
                      ...
                      <p className={`text-2xl font-bold ${(selectedStore as any).growth > 0 ? "text-emerald-600" : "text-destructive"}`}>
                        {(selectedStore as any).growth > 0 ? "+" : ""}{(selectedStore as any).growth.toFixed(1)}%
                      </p>
                    </div>
                  )}
                  ...
                  {(selectedStore as any).rating !== undefined && (selectedStore as any).rating !== null && (
                    <div>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-2xl font-bold">{(selectedStore as any).rating.toFixed(1)}</span>
                      </div>
                    </div>
                  )}
                </div>
