        ...
        {step === "url" && (
          <>
            <div className="flex items-center gap-3 px-6 pt-5 pb-4">
              ...
              <DialogTitle className="text-lg font-bold text-gray-900">Import from URL</DialogTitle>
            </div>
            <div className="px-6 pb-6 space-y-5">
              ...
              <div className="space-y-2">
                ...
                />
                {urlError && <p className="text-[13px] text-red-500 font-medium" data-testid="text-url-error">{urlError}</p>}
              </div>
              <div className="flex justify-center">
                ...
                >
                  {isSubmitting ? (<><Loader2 className="h-4 w-4 mr-2 animate-spin" />Analyzing...</>) : "Analyze URL"}
                </Button>
              </div>
            </div>
          </>
        ...
        {step === "manual" && (
          <>
            <div className="flex items-center gap-3 px-6 pt-5 pb-4 border-b border-gray-100">
              ...
              <DialogTitle className="text-lg font-bold text-gray-900">Setup your product</DialogTitle>
            </div>
            <div className="px-6 py-5 space-y-5 max-h-[65vh] overflow-y-auto">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <label className="text-[13px] font-semibold text-gray-700">Product name <span className="text-red-500">*</span></label>
                  <Input placeholder="e.g., Men's Quarter-Zip Hoodie" value={manualTitle} onChange={(e) => setManualTitle(e.target.value)} className="h-10 bg-white border-gray-200 rounded-lg text-sm" data-testid="input-product-name" />
                </div>
                <div className="space-y-1.5">
              ...
              <div className="space-y-1.5">
                ...
                <Textarea placeholder="e.g., Street-ready hoodie made from soft cotton blend." value={manualDescription} onChange={(e) => setManualDescription(e.target.value.slice(0, 5000))} className="bg-white border-gray-200 rounded-lg text-sm min-h-[100px] resize-none" data-testid="input-product-description" />
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  ...
                  <Input type="number" min="0" step="0.01" placeholder="0.00" value={manualBuyPrice} onChange={(e) => setManualBuyPrice(e.target.value)} className="h-10 bg-white border-gray-200 rounded-lg text-sm" data-testid="input-buy-price" />
                </div>
                <div className="space-y-1.5">
                  <label className="text-[13px] font-semibold text-gray-700">Sell Price ($)</label>
                  <Input type="number" min="0" step="0.01" placeholder="0.00" value={manualSellPrice} onChange={(e) => setManualSellPrice(e.target.value)} className="h-10 bg-white border-gray-200 rounded-lg text-sm" data-testid="input-sell-price" />
                </div>
              </div>
              <div className="space-y-1.5">
                <label className="text-[13px] font-semibold text-gray-700">Image URL</label>
                <Input placeholder="https://example.com/product-image.jpg" value={manualImageUrl} onChange={(e) => setManualImageUrl(e.target.value)} className="h-10 bg-white border-gray-200 rounded-lg text-sm" data-testid="input-image-url" />
              </div>
            </div>
            <div className="flex items-center justify-between gap-3 px-6 py-4 border-t border-gray-100 bg-gray-50/50">
              <Button variant="outline" onClick={handleClose} className="h-10 px-5 rounded-lg text-sm font-semibold" data-testid="button-discard">Discard</Button>
              <Button onClick={handleManualCreate} disabled={isSubmitting || !manualTitle.trim()} className="bg-indigo-600 hover:bg-indigo-700 text-white h-10 px-6 rounded-lg text-sm font-semibold" data-testid="button-create-product">
...
function ViewProductModal({
  ...
  const formatDate = (dateString: string) => {
    ...
    return new Intl.DateTimeFormat("en-US", { month: "long", day: "numeric", year: "numeric", hour: "2-digit", minute: "2-digit" }).format(date)
  }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-2xl p-0 gap-0 overflow-hidden" aria-describedby="view-product-desc">
        <DialogDescription id="view-product-desc" className="sr-only">Product details</DialogDescription>

        <div className="flex items-center justify-between px-6 pt-5 pb-4 border-b border-gray-100">
          <DialogTitle className="text-lg font-bold text-gray-900" data-testid="text-view-product-title">
            Product Details
          </DialogTitle>
        ...
        <div className="px-6 py-5 max-h-[70vh] overflow-y-auto">
          <div className="flex flex-col sm:flex-row gap-6">
            ...
            <div className="flex-1 min-w-0 space-y-4">
              <div>
                <h2 className="text-xl font-bold text-gray-900 leading-tight" data-testid="text-view-product-name">{item.title}</h2>
              ...
              {item.description && (
                <div>
                  <p className="text-sm text-gray-600 leading-relaxed" data-testid="text-view-description">{item.description}</p>
                </div>
              )}
              ...
              <div className="grid grid-cols-3 gap-3">
                <div className="bg-gray-50 rounded-lg p-3 border border-gray-100">
                  <div className="flex items-center gap-1.5 mb-1">
                    ...
                    <span className="text-[11px] font-semibold text-gray-500 uppercase tracking-wide">Buy Price</span>
                  </div>
                  <p className="text-lg font-bold text-gray-900" data-testid="text-view-buy-price">${Number(item.buyPrice).toFixed(2)}</p>
                </div>
                <div className="bg-gray-50 rounded-lg p-3 border border-gray-100">
                  <div className="flex items-center gap-1.5 mb-1">
                    <DollarSign className="h-3.5 w-3.5 text-gray-400" />
                    <span className="text-[11px] font-semibold text-gray-500 uppercase tracking-wide">Sell Price</span>
                  </div>
                  <p className="text-lg font-bold text-gray-900" data-testid="text-view-sell-price">${Number(item.price).toFixed(2)}</p>
                </div>
                <div className={`rounded-lg p-3 border ${profit >= 0 ? "bg-emerald-50/50 border-emerald-100" : "bg-red-50/50 border-red-100"}`}>
                  <div className="flex items-center gap-1.5 mb-1">
                    <DollarSign className="h-3.5 w-3.5 text-gray-400" />
                    <span className="text-[11px] font-semibold text-gray-500 uppercase tracking-wide">Profit</span>
                  </div>
                  <p className={`text-lg font-bold ${profit >= 0 ? "text-emerald-700" : "text-red-700"}`} data-testid="text-view-profit">
              ...
              <div className="flex items-center gap-6 text-sm text-gray-500 pt-1">
                <div className="flex items-center gap-1.5">
                  ...
                  <span data-testid="text-view-added-date">{formatDate(item.addedDate)}</span>
                </div>
                <div className="flex items-center gap-1.5">
                  <Tag className="h-3.5 w-3.5" />
                  <span className="capitalize" data-testid="text-view-source">{item.source.replace(/-/g, " ")}</span>
                </div>
              </div>
            </div>
          </div>
...
function EditProductModal({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg p-0 gap-0 overflow-hidden" aria-describedby="edit-product-desc">
        <DialogDescription id="edit-product-desc" className="sr-only">Edit product details</DialogDescription>
        ...
        <div className="px-6 py-5 space-y-5 max-h-[65vh] overflow-y-auto">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-1.5">
              <label className="text-[13px] font-semibold text-gray-700">Product name <span className="text-red-500">*</span></label>
              <Input value={editTitle} onChange={(e) => setEditTitle(e.target.value)} className="h-10 bg-white border-gray-200 rounded-lg text-sm" data-testid="input-edit-product-name" />
            </div>
            <div className="space-y-1.5">
              <label className="text-[13px] font-semibold text-gray-700">Category</label>
              <Select value={editCategoryId} onValueChange={setEditCategoryId}>
                <SelectTrigger className="h-10 bg-white border-gray-200 rounded-lg text-sm" data-testid="select-edit-category">
          ...
          <div className="space-y-1.5">
            <label className="text-[13px] font-semibold text-gray-700">Product description</label>
            <Textarea value={editDescription} onChange={(e) => setEditDescription(e.target.value.slice(0, 5000))} className="bg-white border-gray-200 rounded-lg text-sm min-h-[100px] resize-none" data-testid="input-edit-description" />
            <p className="text-[11px] text-gray-400 text-right">{editDescription.length}/5000</p>
          </div>

          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1.5">
              <label className="text-[13px] font-semibold text-gray-700">Buy Price ($)</label>
              <Input type="number" min="0" step="0.01" placeholder="0.00" value={editBuyPrice} onChange={(e) => setEditBuyPrice(e.target.value)} className="h-10 bg-white border-gray-200 rounded-lg text-sm" data-testid="input-edit-buy-price" />
            </div>
            <div className="space-y-1.5">
              <label className="text-[13px] font-semibold text-gray-700">Sell Price ($)</label>
              <Input type="number" min="0" step="0.01" placeholder="0.00" value={editSellPrice} onChange={(e) => setEditSellPrice(e.target.value)} className="h-10 bg-white border-gray-200 rounded-lg text-sm" data-testid="input-edit-sell-price" />
            </div>
          </div>

          <div className="space-y-1.5">
            <label className="text-[13px] font-semibold text-gray-700">Image URL</label>
            <Input value={editImageUrl} onChange={(e) => setEditImageUrl(e.target.value)} className="h-10 bg-white border-gray-200 rounded-lg text-sm" data-testid="input-edit-image-url" />
          </div>

          <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg border border-gray-100">
            <div>
              <label className="text-[13px] font-semibold text-gray-700">In Stock</label>
              <p className="text-[12px] text-gray-500 mt-0.5">Toggle availability status for this product</p>
            </div>
            <Switch checked={editInStock} onCheckedChange={setEditInStock} data-testid="switch-edit-in-stock" />
          </div>
        </div>

        <div className="flex items-center justify-between gap-3 px-6 py-4 border-t border-gray-100 bg-gray-50/50">
          <Button variant="outline" onClick={() => onOpenChange(false)} className="h-10 px-5 rounded-lg text-sm font-semibold" data-testid="button-edit-cancel">Cancel</Button>
          <Button onClick={handleSave} disabled={isSubmitting || !editTitle.trim()} className="bg-indigo-600 hover:bg-indigo-700 text-white h-10 px-6 rounded-lg text-sm font-semibold" data-testid="button-edit-save">
...
export default function MyProductsPage() {
  ...
  const fetchPicklist = async () => {
    try {
      const response = await apiFetch("/api/picklist", { credentials: 'include' })
      if (!response.ok) {
        if (response.status === 401) { setItems([]); return }
  ...
  const handleRemove = async (id: string) => {
    try {
      const response = await apiFetch(`/api/picklist/${id}`, { method: 'DELETE' })
              ...
              <div className="text-center">
                ...
                {!searchQuery && (
                  <div className="flex items-center justify-center gap-3">
                    <Button onClick={() => setAddModalOpen(true)} className="bg-indigo-600 hover:bg-indigo-700 text-white" data-testid="button-add-product-empty">
                      <Plus className="h-4 w-4 mr-1.5" />Add Product
                    </Button>
                    <Button variant="outline" asChild><a href="/products/winning-products">Browse Products</a></Button>
                  </div>
                )}
              </div>
            </Card>
