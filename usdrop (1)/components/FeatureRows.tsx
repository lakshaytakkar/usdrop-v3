               ...
               <div className="grid grid-cols-2 gap-4 p-4 bg-white rounded-3xl shadow-2xl border border-slate-100 transform rotate-1 hover:rotate-0 transition-transform duration-500">
                  ...
                  {productCategories.slice(0, 4).map((category, i) => (
                     <div key={i} className="aspect-square bg-slate-50 rounded-xl overflow-hidden relative group border border-slate-100">
                        <GeneratedImage 
                           prompt={`commercial product photography of ${category}, trending item, studio lighting, white background`}
                           className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700"
                           alt={category}
                        />
                        <div className="absolute bottom-2 left-2 right-2 bg-white/90 backdrop-blur px-3 py-1.5 rounded-lg text-xs font-bold text-slate-900 flex justify-between">
            ...
            <div className="w-full md:w-1/2 order-2 md:order-1 relative">
               ...
               <div className="relative h-[600px] w-full bg-slate-900 rounded-[2.5rem] border border-slate-800 shadow-2xl flex items-center justify-center overflow-hidden p-8">
                  ...
                  <div className="relative w-full max-w-sm">
                     <div className="bg-slate-800 rounded-2xl p-4 mb-4 border border-slate-700 animate-in slide-in-from-bottom-8 duration-700">
                        ...
                        <div className="aspect-video bg-slate-900 rounded-lg overflow-hidden relative">
                           <GeneratedImage prompt="tiktok style product video thumbnail, engaging hook text overlay, bright colors" className="w-full h-full object-cover opacity-80" />
                        </div>
                     </div>
                     
                     <div className="bg-slate-800 rounded-2xl p-4 border border-slate-700 animate-in slide-in-from-bottom-8 duration-700 delay-200 opacity-60 scale-95">
                         <div className="flex items-center gap-3 mb-3">
