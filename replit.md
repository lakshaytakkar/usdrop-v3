...
- **Feb 2026**: Visual overhaul — background, font, and sub-nav. Background changed from blueish gradient to flat `#F5F5F7` across `AppLayout` and sticky header. Font switched from DM Sans to General Sans (via Fontshare CDN, DM Sans kept as fallback) in both `@theme` and `:root` CSS variables. Sub-nav tabs redesigned from glass-morphism bubble style (blur + semi-transparent white) to Creatify-style card selectors: white cards with subtle border on gray background, no blur effects. Top nav also updated from glass-morphism to clean white bg with minimal shadow. Quick filters updated to match card style.
- **Feb 2026**: LLC page and Private Supplier page redesigned to match Creatify-style FeaturePageTemplate. Both pages now use: bold headlines (`text-[40px] md:text-[52px]`), tight tracking, minimal text, generous spacing (`space-y-20 lg:space-y-28`), `bg-[#FAFAFA]` cards with `border-black/[0.04]`, dark CTA banner (`bg-[#0A0A0A] rounded-[24px]`), radial gradient glow overlay. All glass-morphism removed (no backdrop-blur, no bg-white/60). LLC page: removed LLC Status section (empty placeholder data), removed Badge pills, added multiple CTAs throughout, discount banner simplified to white card. Private Supplier page: stats use large `text-[28px]` values, How It Works uses numbered steps with faded step numbers, contact card (Juice Chen) cleaned up, all feature cards use uniform gray icon backgrounds.
